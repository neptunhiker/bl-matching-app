...
@transaction.atomic
def send_clarification_need_info_to_coach_email(matching_attempt, triggered_by: str="system"):
    """Send an email to the coach to inform them about after the intro call the participant requested a clarification of the matching instead of starting coaching."""
    ...
    transaction.on_commit(
        lambda: send_email(
            to=coach.user.email,
            subject=f"Klärungsbedarf nach Kennenlerngespräch mit Dir",
            template_name='emails/info_clarification_need_to_coach.html',
            context=context,
            matching_attempt=matching_attempt,
            sent_by=context["author"],
            triggered_by=triggered_by
