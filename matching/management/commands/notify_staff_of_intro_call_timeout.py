...
class Command(BaseCommand):
    help = (
        "Notify BL staff via Slack when a coach has not organised an intro call "
        "even after receiving a reminder. Staff should escalate manually. "
    ...
    def handle(self, *args, **options):
        ...
        pending = (
            ...
            .eligible_for_intro_call_staff_escalation()
            .select_related("matched_coach", "participant", "bl_contact")
            .order_by("intro_call_deadline_at")
        )[:self.MAX_PER_RUN]

        if verbosity >= 1:
            self.stdout.write(f"Found {len(pending)} attempt(s) eligible for staff escalation.")
                ...
                if verbosity >= 2:
                    self.stdout.write(
                        self.style.SUCCESS(
                            ...
                            f"(coach: {attempt.matched_coach}, participant: {attempt.participant})"
                        )
                    )
            except Exception as exc:
                failed += 1
                logger.exception("Error sending staff escalation for attempt %s", attempt.id)
                self.stderr.write(
