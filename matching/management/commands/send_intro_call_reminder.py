...
from matching.utils import get_intro_call_extension_deadline
...
class Command(BaseCommand):
    help = (
        "Send a one-time reminder to coaches who have not organised an intro call "
        "by their deadline. Extends the deadline by one working day. "
    ...
    def handle(self, *args, **options):
        ...
        if verbosity >= 1:
            self.stdout.write(f"Found {len(pending)} attempt(s) eligible for an intro call reminder.")
        ...
        for attempt in pending:
            try:
                with transaction.atomic():
                    attempt.intro_call_deadline_at = get_intro_call_extension_deadline(
                    ...
                    create_matching_event(
                        ...
                        payload={"deadline_at": attempt.intro_call_deadline_at.isoformat()},
                    )
                sent += 1
                if verbosity >= 2:
                    self.stdout.write(
                        self.style.SUCCESS(
                            ...
                            f"(coach: {attempt.matched_coach}, new deadline: {attempt.intro_call_deadline_at})"
                        )
                    )
            except Exception as exc:
                failed += 1
                logger.exception("Error sending intro call reminder for attempt %s", attempt.id)
                self.stderr.write(
